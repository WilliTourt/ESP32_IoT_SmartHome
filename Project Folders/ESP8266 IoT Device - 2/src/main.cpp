...
void setup() {
	...
	server.on("/light1", livingLight);
	server.on("/light2", diningLight);
	server.on("/lightTask1", lightTask1);
	server.on("/lightTask2", lightTask2);
...
void livingLight() {
	...
	(living.read() == HIGH) ? (living.off()) : (living.on());
	state = (living.read() == HIGH) ? ("on") : ("off");
...
void diningLight() {
	...
	(dining.read() == HIGH) ? (dining.off()) : (dining.on());
	state = (dining.read() == HIGH) ? ("on") : ("off");
